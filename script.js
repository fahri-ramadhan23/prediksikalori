...
const weights = { intercept: -315, gender: 15.5, age: -0.5, weight: 4.5, duration_hour: 415, bpm: 2.7 };
...
function startPrediction() {
  ...
  btn.disabled = true;
  btnText.textContent = "Menganalisis Data...";
...
function calculate() {
  ...
  // Konversi menit ke jam untuk rumus regresi (karena weights berbasis jam)
  const durationInHours = durationTotalMinutes / 60;

  let pred = weights.intercept + (gender * weights.gender) + (age * weights.age) + (weight * weights.weight) + (durationInHours * weights.duration_hour) + (bpm * weights.bpm);
  
  const resultValue = Math.round(Math.max(0, pred));
